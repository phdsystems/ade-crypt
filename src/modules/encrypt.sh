...
encrypt_file() {
    ...
    if compress_data "${compression}" < "${input_file}" | \
        openssl enc -aes-256-cbc -salt -out "${output_file}" -pass file:"${key_file}"; then
...
encrypt_password() {
    ...
    if compress_data "${compression}" < "${input_file}" | \
        gpg --symmetric --cipher-algo AES256 --output "${output_file}"; then
...
encrypt_two_factor() {
    ...
    if gpg --symmetric --cipher-algo AES256 --output "${output_file}" "${temp_file}"; then
        # Cleanup
        shred -vzu "${temp_file}" 2>/dev/null || rm -f "${temp_file}"
...
encrypt_directory() {
    ...
    if tar czf - "${dir}" | openssl enc -aes-256-cbc -salt -out "${output}" -pass file:"${key_file}"; then
